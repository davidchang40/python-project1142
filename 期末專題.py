...
def _reset_hypothesis_globals():
    ...
    global n_samples, tail_type, alpha, raw_data, proportion, p0
    global mu, x_bar, sigma, n, x_bar2, sigma2, df
...
def get_critical_value():
    if is_proportion or is_population_known:
        if tail_type == "left":
            return scipy_stats.norm.ppf(alpha)
        elif tail_type == "right":
            return scipy_stats.norm.ppf(1 - alpha)
        else:
            return scipy_stats.norm.ppf(1 - alpha / 2)
    else:
        if tail_type == "left":
            return scipy_stats.t.ppf(alpha, df)
        elif tail_type == "right":
            return scipy_stats.t.ppf(1 - alpha, df)
        else:
